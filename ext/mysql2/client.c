...
static VALUE rb_raise_mysql2_error(mysql_client_wrapper *wrapper) {
  ...
  /* If the server is < MySQL 5.5, the encoding is not usable. set to BINARY */
  if (wrapper->server_version < 50500) {
    int binary_enc = rb_enc_find_index("BINARY");
    rb_enc_associate_index(rb_error_msg, binary_enc);
    rb_enc_associate_index(rb_sql_state, binary_enc);
  } else {
